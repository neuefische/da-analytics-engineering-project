...
WITH departures AS (
    ...
    WHERE origin IN (SELECT DISTINCT airport_code FROM {{ ref('prep_weather_daily_join_origin') }})
...
arrivals AS (
    ...
    WHERE dest IN (SELECT DISTINCT airport_code FROM {{ ref('prep_weather_daily_join_dest') }})
...
ON r.airport = pw.airport_code AND r.flight_date::date= pw.date